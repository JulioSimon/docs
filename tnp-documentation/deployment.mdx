---
title: 'Deployment'
description: 'Comprehensive guide for deploying the Tonga National Portal to production environments'
icon: 'server'
---
...
<CardGroup cols={2}>
  <Card title="CPU" icon="microchip">
    **Minimum**: 4 cores  
    **Recommended**: 8+ cores for better performance
  </Card>
  <Card title="RAM" icon="memory">
    **Minimum**: 16GB  
    **Recommended**: 32GB for larger installations
  </Card>
  <Card title="Storage" icon="hard-drive">
    **Minimum**: 100GB SSD  
    **Recommended**: 250GB+ SSD with RAID configuration
  </Card>
  <Card title="Network" icon="network-wired">
    **Minimum**: High-speed internet connection with 1Gbps bandwidth  
    **Recommended**: Redundant connections for high availability
  </Card>
</CardGroup>
...
<Card title="Recommended OS" icon="ubuntu">
...
<Steps>
  ...
  <Step title="PHP 8.2 Installation">
  ...
  <Step title="Web Server Installation">
  ...
  <Step title="Database Server Installation">
...
<Callout type="warning">
  Implementing security measures is critical to protect your server from unauthorized access and potential attacks.
</Callout>

<AccordionGroup>
  <Accordion title="Secure SSH Configuration">
    ```bash
  ...
  <Accordion title="Install and Configure Fail2ban">
    ```bash
    # Install fail2ban
  ...
  <Accordion title="Secure Shared Memory">
    ```bash
    # Edit fstab
    sudo nano /etc/fstab
    ```
...
<Card title="MySQL Configuration" icon="database">
...
<Steps>
  ...
  <Step title="Create WordPress Database and User">
  ...
  <Step title="MySQL Performance Optimization">
...
<Card title="WordPress Setup" icon="wordpress">
  This section covers downloading, configuring, and securing WordPress 6.5+.
</Card>

<Steps>
  <Step title="Download and Extract WordPress">
  ...
  <Step title="WordPress Configuration">
  ...
  <Step title="Secure wp-config.php">
...
<Steps>
  ...
  <Step title="HTTPS Configuration (For After SSL Certificate Installation)">
    <Note>
      The following configuration should be added after obtaining an SSL certificate. It's included here for reference.
    </Note>
  ...
  <Step title="PHP-FPM Optimization">
  ...
  <Step title="Verify Web Server Setup">
...
<Card title="SSL Security" icon="lock">
  Secure communication between users and the Tonga National Portal is essential for protecting sensitive information and maintaining trust.
</Card>
...
<Steps>
  ...
  <Step title="Obtain and Install Certificate">
    ```bash
    # Obtain and install certificate for the www subdomain only
    sudo certbot --nginx -d www.gov.to
    ```
    
    Follow the interactive prompts to complete the certificate issuance process.
  </Step>
...
<AccordionGroup>
  <Accordion title="Verify the auto-renewal timer is active">
    ```bash
  ...
  <Accordion title="Manually configure auto-renewal">
  ...
  <Accordion title="Test the renewal process">
    ```bash
  ...
  <Accordion title="Best practices for reliable renewals">
...
<Card title="Pre-configured Package Deployment" icon="box-open">
  For the Tonga National Portal, a pre-configured WordPress package will be provided to the client. This package includes a zip archive containing all server files and a complete database backup.
</Card>

<Steps>
  <Step title="Deploy File System">
    ```bash
  ...
  <Step title="Update wp-config.php">
...
<Card title="DNS Setup" icon="globe">
...
<Tabs>
  <Tab title="Primary Domain Records">
    ```
    # A Records (IPv4)
  ...
  <Tab title="Email-Related Records">
  ...
  <Tab title="Additional Service Records">
...
<Steps>
  <Step title="Log in to your domain registrar's control panel">
  ...
  <Step title="Locate the DNS management section">
    - This might be called "DNS Settings," "Name Servers," or "Domain Management"
  </Step>
  
  <Step title="Add or modify A records">
    ```
  ...
  <Step title="Add MX records for email (if applicable)">
    ```
    Type: MX
    Host/Name: @ (or leave blank)
    Priority: 10
  ...
  <Step title="Add TXT records for email authentication and verification">
    ```
    Type: TXT
    Host/Name: @ (or leave blank)
...
<AccordionGroup>
  ...
  <Accordion title="Verify A records">
    ```bash
  ...
  <Accordion title="Verify MX records">
    ```bash
    # Check MX records
    dig +short gov.to MX
  ...
  <Accordion title="Verify TXT records">
    ```bash
    # Check TXT records
  ...
  <Accordion title="Online DNS verification tools">
...
<CardGroup cols={2}>
  <Card title="Use www and non-www versions" icon="globe">
    Configure both www.gov.to and gov.to to point to your server and choose one as canonical
  </Card>
  <Card title="Set appropriate TTL values" icon="clock">
    Use lower TTL values (300-900 seconds) before planned changes and higher values (3600+ seconds) for stable sites
  </Card>
  <Card title="Implement DNSSEC" icon="shield-halved">
    Enable DNSSEC to prevent DNS spoofing if supported by your registrar
  </Card>
  <Card title="Monitor DNS health" icon="chart-line">
    Set up monitoring for your DNS records to detect unauthorized changes
  </Card>
  <Card title="Document your DNS configuration" icon="file-lines">
    Maintain documentation of all DNS records and their purposes
  </Card>
  <Card title="Use DNS redundancy" icon="server">
...
<AccordionGroup>
  <Accordion title="Authorization requirements">
  ...
  <Accordion title="DNS management responsibility">
...
<Card title="Performance Optimization" icon="bolt">
  Implementing effective caching is critical for optimizing WordPress performance, especially for a government portal like the Tonga National Portal that needs to serve content efficiently to citizens.
</Card>
...
<Steps>
  <Step title="Redis Object Cache Setup">
  ...
  <Step title="WordPress Redis Integration">
  ...
  <Step title="Verifying Redis Object Cache">
...
<Tabs>
  <Tab title="Nginx FastCGI Cache">
    <Steps>
      ...
      <Step title="Update Nginx Server Block">
        ```bash
        sudo nano /etc/nginx/sites-available/tongaportal
        ```
  ...
  <Tab title="WP Super Cache">
    <Steps>
      <Step title="Install WP Super Cache">
        ```bash
      ...
      <Step title="Configure WP Super Cache">
      ...
      <Step title="Set Up Cache Preloading">
        ```bash
...
<Steps>
  <Step title="Configure Nginx for Browser Caching">
    ```bash
    sudo nano /etc/nginx/sites-available/tongaportal
    ```
  ...
  <Step title="WordPress Cache-Control Headers">
...
<Card title="Verification Process" icon="check">
...
<Steps>
  <Step title="Server Resource Monitoring">
  ...
  <Step title="Service Status Verification">
...
<Steps>
  <Step title="WordPress Version and Updates">
  ...
  <Step title="WordPress Configuration Check">
  ...
  <Step title="Database Integrity Check">
...
<Tabs>
  <Tab title="Core WordPress Functionality">
    <CardGroup cols={2}>
      <Card title="Admin Login" icon="user-lock">
        Navigate to `/wp-admin` and login with admin credentials
      ...
      <Card title="Content Creation" icon="file-lines">
        ...
        **Expected Result**: Content saves and displays correctly
      </Card>
      
      <Card title="Media Upload" icon="images">
        Upload test images and documents
        
        **Expected Result**: Media uploads successfully and displays in media library
      </Card>
      
      <Card title="Search Function" icon="magnifying-glass">
        Perform searches with various keywords
        
        **Expected Result**: Relevant results appear
      </Card>
      
      <Card title="User Management" icon="users">
        Create a test user account
        
        **Expected Result**: Account created with proper role and permissions
      </Card>
    </CardGroup>
  </Tab>
  
  <Tab title="Custom Functionality">
    <AccordionGroup>
      <Accordion title="Government Ministries and Services Directory">
        - Verify all ministries and services listings are accurate and accessible
        - Confirm contact information is correct
      </Accordion>
      
      <Accordion title="Document Repository">
        - Test document uploads and downloads
        - Verify document categorization and search
      </Accordion>
      
      <Accordion title="News and Announcements">
      ...
      <Accordion title="MxChat AI Chatbox">
...
<Steps>
  <Step title="WordPress Security Scan">
  ...
  <Step title="SSL/TLS Configuration Verification">
  ...
  <Step title="File and Directory Permissions Audit">
  ...
  <Step title="Security Headers Implementation">
    ...
    <Warning>
      A bad implementation of the Content-Security-Policy can lead to broken website display. Make sure you reference all the scripts loaded by the different plugins of the installation and other scripts that are needed to render the website successfully.
    </Warning>
  </Step>
  
  <Step title="Firewall and Security Plugin Verification">
  ...
  <Step title="Database Security Audit">
...
<Steps>
  <Step title="Backup System Verification">
  ...
  <Step title="Document Rollback Procedure">
    Document the complete rollback procedure for future reference:
    
    <AccordionGroup>
      <Accordion title="Database Rollback Procedure">
        ```bash
        # Stop web server
      ...
      <Accordion title="File System Rollback Procedure">
        ```bash
        # Stop web server
        sudo systemctl stop nginx
      ...
      <Accordion title="Complete System Rollback Procedure">
        ```bash
        # Stop all services
...
<Card title="Deployment Summary" icon="check">
...
<Note>
  Remember to regularly review and update the deployment as WordPress, plugins, and server software evolve. Security patches should be applied promptly, and performance should be monitored to ensure the portal continues to meet the needs of Tonga's citizens.
</Note>
